...
import SchoolForm from '../../components/pages/add-location/school-form';
...
describe('SchoolForm', () => {
  ...
  it('renders correctly', () => {
    const wrapper = shallow(<SchoolForm {...props} />);
  ...
  it('onSubmit', async () => {
    ...
    jest.spyOn(Hooks, 'useActionDispatch').mockReturnValue(manageLocation);
    const getForm = jest.spyOn(ReactFormDynamic, 'useForm');
    shallow(<SchoolForm {...props} />);
    const [{ onSubmit }] = getForm.mock.calls[0];
    onSubmit({ zip: '90210' });
    await flushPromises();
    expect(history.push).toHaveBeenCalledWith({ 'data': { 'isFirstTime': true, locationType: 'School' }, 'pathname': '/location/id/' });
    getForm.mockRestore();
  });
