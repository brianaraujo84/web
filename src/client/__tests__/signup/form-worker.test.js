...
import Form from '../../components/pages/signup/form-worker';
...
describe('SignupForm', () => {
  ...
  it('PinInput', () => {
    ...
    jest.spyOn(ReactFormDynamic, 'useForm').mockReturnValueOnce(formik);
    const wrapper = shallow(<Form {...props} />);
    const handlePinChange = wrapper.find('PinInput').invoke('onChange');
    handlePinChange('123');
    expect(formik.setFieldValue).toHaveBeenCalledWith('pin', '123');
  });
});
