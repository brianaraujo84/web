...
import { postConfidenceJobObject, postConfidenceObject, postTaskCommunicationObject, setObject } from '../../../redux/actions/object';
...
import AddImageModal from '../task-details/add-image-modal';
import TaskStartModal from './task-start-modal';
import { DateUtils, toBase64Array, dataUrlToFile, TextUtils } from '../../../utils';
import * as URLS from '../../../urls';
import { TaskStatus, StorageKeys, COMMENT_ADDED_EVENT } from '../../../constants';
import { getItem } from '../../../utils/storage-utils';
import DeclineJobModal from './decline-job-modal';
import { formatTaskRecurringType } from '../../../lib/task';
import SimpleWorkerTaskCardSkin from './skins/simple-worker-task-card-skin';
...
const WorkerTaskCard = ({ task, getLocation, updateTaskInNonGrpJobTasks, isOnTaskDetailsPage, updateTaskGroup, isReadOnly, locationUserRole, index, updateFilters, cardSkin, insertCopiedTask, taskSummary, isNotifyPage }) => {
  ...
  const handleCopy = () => {
    ...
    if (!taskSummary.templateId || copyTask.taskType === 'CustomGroupTask') {
  ...
  const handleTaskGroupRouting = () => {
    if ((locationUserRole === 'Owner' || locationUserRole === 'Manager') && (task.assigneeUserName !== profile.username || isReviewTask)) {
      history.push({ pathname: URLS.TASK_DETAILS(locationId, task.templateId), search: TextUtils.generateQS(task), state: { templateType: 'Custom' } });
    } else {
      history.push({ pathname: URLS.TASK_DETAILS(locationId, task.templateId, 'tm'), search: TextUtils.generateQS(task), state: { templateType: 'Custom' } });
    }
  };
  ...
  return (
    <>
      {cardSkin === 'simple' &&
        ...
        />}
      {!cardSkin && <div
        className={`task alert p-0 alert-${cardColor} ${task.groupCard ? 'group' : ''}`}
      >
        {
          (task.stage && !isJob) &&
          <a
            className={classnames(
              [
                'p-1 text-center task-status text-white rounded-left'
              ]
            )}
          >
            {t(task.stage.replace(/\s/g, ' '))}
          </a>
        }
        {
          (task.stage && isJob) &&
          <span className="p-1 task-status align-self-stretch rounded-left flex-grow-0"></span>
        }
        ...
        <div className="p-2">
          ...
          {!showTaskDetails && task.taskStartedTime && <hr className="mt-2 mb-1" />}
          {!showTaskDetails && task.taskStartedTime && <div className="mb-1"><small><i className="far fa-calendar-alt lineitem-icon" aria-hidden="true"></i> Started: {DateUtils.unicodeFormat(DateUtils.parseISO(task.taskStartedTime), 'P p')}</small></div>}
          {!showTaskDetails && task.taskCompletedTime && <div className="mb-1"><small><i className="far fa-calendar-check lineitem-icon" aria-hidden="true"></i> Completed: {DateUtils.unicodeFormat(DateUtils.parseISO(task.taskCompletedTime), 'P p')}</small></div>}
          {!showTaskDetails && task.taskDurationTime && <div className="mb-1"><small><i className="far fa-clock lineitem-icon" aria-hidden="true"></i> Duration: {task.taskDurationTime}</small></div>}
          {!showTaskDetails && task.taskApprovedTime && !isInProgressTask && <div className="mb-1"><small><i className="far fa-check lineitem-icon" aria-hidden="true"></i> Approved: {DateUtils.unicodeFormat(DateUtils.parseISO(task.taskApprovedTime), 'P p')}</small></div>}
          {!showTaskDetails && (task.taskApprovedTime || task.taskCompletedTime) && lastComment?.comments?.length > 0 && lastComment?.totalCount && <hr className="mt-2 mb-1" />}
          ...
          <div className="text-right">
                ...
                <>
                  ...
                  {taskActivityImages.length === 0 && task.imageRequired === 2 &&
                    <Button
                      ...
                      onClick={() => handleAddScreenshotButtonClick(false)}
                    >
            ...
            {(isCompletedTask && (task.taskType === 'Adhoc' || task.taskType === 'CustomGroupTask') && (locationUserRole === 'Owner' || locationUserRole === 'Manager')) &&
              <Button
                className="task-c-button p-1 ml-1 btn rounded task-detail-toggle"
                variant="outline-secondary"
                data-target="copy-task-details"
                onClick={handleCopy}
              >
                <i aria-hidden="true" className="far fa-copy"></i>
                <span className="sr-only"><Trans>Copy Task</Trans></span>
              </Button>}
            ...
            {openMenuPopup && <EllipsisMenuV3
              ...
              isAddFileActive={!isReadOnly && isInProgressTask && (task.taskType === 'Adhoc' || task.taskType === 'CustomGroupTask') && !isCustomJobGroupCard}
              addFile={() => handleTaskFileClick()}
              isSetTaskLinkActive={!isReadOnly && isInProgressTask && (task.taskType === 'Adhoc' || task.taskType === 'CustomGroupTask') && !isCustomJobGroupCard}
              setTaskLink={() => handleSetTaskLinkClick()}
              isAddScreenshotsActive={!isReadOnly && isInProgressTask && (task.taskType === 'Adhoc' || task.taskType === 'CustomGroupTask') && !isCustomJobGroupCard}
            ...
            {!isTemplateJob && !isCustomJobGroupCard && isExpandable && <Button
              className="task-c-button p-1 ml-1 rounded"
              variant={isCompletedTask ? 'outline-secondary' : 'outline-primary'}
