...
import RejectTaskModal from '../location-details/reject-task-modal';
import RejectJobModal from '../location-details/reject-job-modal';
import AssignTaskModal from '../location-details/assign-task-modal';
...
import AssociateZoneModal from '../location-details/associate-zone-modal';
import { getTaskImages, getJobActivityImages, uploadFileToDbxV2, getFilePathFromDbx, copyFileFromDbx } from '../../../redux/actions/files';
import { DateUtils, toBase64Array, smoothScrollToRef, dataUrlToFile } from '../../../utils';
...
import SimpleTaskCardSkin from '../location-details/simple-task-card-skin';
...
const TaskCardDragable = ({
  ...
  const copyTaskGroup = useActionDispatch(postConfidenceObject(OBJECT_TEMPLATE));
  ...
  const handleUpdateTask = async (data) => {
    if (task.groupCard && (task.templateType === 'Custom' || task.templateType === 'Main')) {
      ...
      const ts = { ...data };
      ts.taskRecurring && (ts.taskRecurring.nextOccurrenceDate = `${ts.taskRecurring.startDate} ${ts.taskRecurring.startTime}`);
      updateTaskInNonGrpJobTasks(task.taskId, { ...task, ...ts }, 'update');
      await manageTask(data);
    }
  ...
  const handleAssignTask = async (data, skipUpdate = false) => {
    if (isCustomJobGroupCard) {
      ...
      updateTaskInNonGrpJobTasks(task.taskId, { ...task, assigneeName }, 'update');
  ...
  // COMMENTING THIS OUT UNTIL WE FIND A BETTER WAY TO HANDLE FOCUS
  ...
  const handleSaveTitle = async () => {
    ...
    if (('Todo' === templateType || taskTemplateId || !task.groupCard) && !task.custom) {
      task.task = taskTitle;
      updateTaskInNonGrpJobTasks(task.taskId, { ...task }, 'update');
      await manageTask(data);
      if (task.taskType === 'CustomGroupTask') {
        updateTasks();
      }
  ...
  const handleRequireVerificationSelectItem = async (value) => {
    ...
    updateTaskInNonGrpJobTasks(task.taskId, { ...task, imageRequired: data.imageRequired }, 'update');
    await manageTask(data);
    updateTasks();
  };
  ...
  return (
    <>
      {cardSkin === 'simple' &&
        ...
        />}
      {!cardSkin && !task.isDeleted && <Draggable draggableId={task.taskId.toString()} index={index} isDragDisabled={!sortingMode} disableInteractiveElementBlocking={!sortingMode}>
        {(provided) => (
          <div
            ref={provided.innerRef} {...provided.draggableProps}
            className={classnames(['task alert p-0',
              `alert-${task.stage === 'Review' && customJobReviewCompleted ? 'dark' : cardColor} ${selectedForBulk ? 'selected' : undefined}`,
          ...
          >
            {
              ...
              >
                ...
                <i {...provided.dragHandleProps} className="fas fa-lg fa-grip-vertical p-2 fade-0" aria-hidden="true"></i>
              </a>
            }
            {
              (task.stage && isJob) &&
            ...
            <div className="p-2">
              ...
              {task && task.taskRecurring && task.taskRecurring.nextOccurrenceDate && !isCustomJobGroupCard && !isInProgressTask && !isReviewTask && !isCompletedTask && !isIncompleteTask &&
                <p className='mb-0'>
                  <small>
              ...
              {lastComment?.comments?.length > 0 && lastComment?.totalCount && <p className="mb-0">
                <small>
                  ...
                  <Link onClick={handleAddComment} href='#'>{t(`${lastComment.totalCount} comment${lastComment.totalCount > 1 ? 's' : ''}`)}</Link>
              ...
              <div className="text-right">
                <div>
                  ...
                  {isCloneTask &&
                    <button
                      type="button"
                      className="task-c-button task-copy-create p-1 ml-2 btn rounded btn-info"
                  ...
                  {isOwnerOrManager() && !isReviewTask && !isCompletedTask && !isIncompleteTask && (<>
                    {((!isCustomJobGroupCard && isTemplateJobGroupCard && !task.deviceName) || (!isCustomJobGroupCard && !isTemplateJobGroupCard)) && <Button
                      type="button"
                      variant="outline-primary"
                    ...
                    {openMenuPopup && <EllipsisMenuV3
                      ...
                      isAddImagesActive={!isJob && isEditable && taskImages.length < MAX_ALLOWED_IMAGES && !isCloneTask && !isTemplateJobGroupCard}
                      handleAddImageButtonClick={handleAddImageButtonClick}
                      isAddScreenshotsActive={!isJob && isEditable && taskImages.length < MAX_ALLOWED_IMAGES && !isCloneTask}
                      handleAddScreenshotButtonClick={handleAddScreenshotButtonClick}
                      isCopyActive={isJob ? false : true}
                      handleCopy={handleCopy}
                      isAssociateActive={task?.templateType !== 'Main' && !isCustomJob && locationZones && locationZones.length > 0}
                      ...
                      isCopyGroupActive={!isCustomJobGroupCard && !isTemplateJobGroupCard && task.taskType !== 'CustomGroupTask'}
                      handleCopyGroupClick={handleCopyGroupClick}
                      isDeleteTaskActive={!isReviewTask && !isCompletedTask && !isIncompleteTask && !isCustomJobGroupCard && !isTemplateJobGroupCard}
                      handleDeleteTask={handleRemoveClick}
                  ...
                  {isJob ?
                    ...
                    (!isReviewTask || expandReviewTask) && isExpandable && (
                      <Button
      ...
      {showTaskReviewModal && <AssignedModal
        ...
        onConfirm={() => changeFiltersTo(sortingMode ? 'Task Order' : 'Active')}
...
TaskCardDragable.displayName = 'TaskCard';
export default TaskCardDragable;
